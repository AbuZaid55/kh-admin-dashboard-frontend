...
export default function AddProduct() {
  ...
  const removeDiamondPcs = (diamondIndex, pcsIndex) => {
    ...
    setProduct((prev) => {
      const updatedDiamonds = prev.diamonds.map((diamond, i) => (i === diamondIndex ? { ...diamond, pcs: diamond.pcs.filter((_, i) => i !== pcsIndex) } : diamond));
      return { ...prev, diamonds: updatedDiamonds };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/store/eshop/products/add-product", formData, {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex gap-2 border border-gray-300 shadow-sm px-4 py-2 rounded-md my-6">
          ...
          <Input
            ...
            value={product.isItRing}
            type="checkbox"
            name="isItRing"
            onChange={(e) => {
              setProduct({ ...product, isItRing: e.target.value });
            }}
          />
        </div>
