...
function HallmarkAndJewel({ formdata, setformdata, handleform, tittle }) {
  ...
  return (
    <>
      <OnOffbutton isOn={isOn} setIsOn={setIsOn} openclosebutton={openclosebutton} setopenclosebutton={setopenclosebutton} headname={tittle}>
            ...
            <div className="flex items-center flex-wrap gap-2 mt-2 space-x-4">
              {images.map((_, index) => (
                <label key={index} className="w-[120px] h-[120px] border border-[#e7e7e7] rounded-md flex items-center justify-center cursor-pointer hover:bg-gray-50">
                  {formdata.images[index] ? <img src={URL.createObjectURL(formdata.images[index])} className=" w-full h-full object-cover" /> : <Input type="file" className="hidden" onChange={(e) => handleImageChange(e, index)} />}
                </label>
              ))}
              {images.length && (
