...
function AssaiAddImage() {
  ...
  const handleform = (e) => {
    ...
    if (editSlideIndex !== null) {
      const editedSlides = slides.map((slide1, index) => (index === editSlideIndex ? formdata : slide1));
  ...
  return (
    <>
      <OnOffbutton isOn={isOn} setIsOn={setIsOn} openclosebutton={openclosebutton} setopenclosebutton={setopenclosebutton} headname="Asai Images">
        <form className=" mt-5" onSubmit={handleform}>
          <label>Style Name</label>
          <Input placeholder="Enter Style Name" value={formdata.Style_Name} onChange={(e) => setformdata({ ...formdata, Style_Name: e.target.value })} />
          ...
          <div className=" mt-5 flex gap-[100px]">
            <div>
              ...
              <label className="w-[120px] h-[120px] border border-[#e7e7e7] rounded-md flex items-center justify-center text-gray-500 hover:bg-gray-50 cursor-pointer mt-2 ">
                {formdata.Image ? (
                  ...
                  <div>
                    <FaPlus />
                    <input name="img" required type="file" className="hidden" accept="image/*" onChange={(e) => setformdata({ ...formdata, Image: e.target.files[0] })} />
                  </div>
                )}
              </label>
            </div>
            ...
            <div className=" flex flex-col gap-4">
              ...
              <div className=" flex justify-end gap-[30px] w-full mt-6 ">
                <div className="w-[15%]">
                  <Button className="bg-[#37D160] hover:bg-transparent border-[#37D160] w-full hover:text-[#37D160]" text="Add Carousel" />
                </div>
