...
const BASE_TESTIMONIAL_URL = 'http://localhost:3000/khwaahish/customization/homepage/khw-testimonial';
...
//   const [selectedCollection, setSelectedCollection] = useState('');
  ...
  const fetchCollections = async () => {
    try {
      const response = await axios.get('http://localhost:3000/store/khw/collections/get-all-collections');
      
      const collectionsData = response.data.collections ? response.data.collections : response.data;
  ...
  const fetchTestimonials = async () => {
    try {
      ...
      const testimonialsData = response.data.testimonials ? response.data.testimonials : response.data;
  ...
  return (
    <>
      <div className='flex flex-col gap-6 p-4'>
        ...
        {error && <div className='bg-red-100 p-3 text-red-700 rounded-md mb-4'>{error}</div>}
        ...
        <form onSubmit={handledata}>
          ...
          <div className='mt-3'>
            ...
            >
              {formdata.profile_img ? (
                ...
                />
              ) : editId && slides.find(s => s._id === editId)?.profile_img?.url ? (
                <img 
          ...
          <div className='flex justify-end gap-[22px] mt-6'>
            ...
            <div className='w-[20%]'>
              <Button 
                text={loading ? "Saving..." : (editId ? "Update" : "Save Changes")} 
                type='submit'
                disabled={loading} 
              />
            </div>
      ...
      <div className='mt-6'>
        ...
        <div className='flex flex-col gap-10 mb-3 overflow-auto py-1 scrollbar-hide'>
          {Array.isArray(slides) && slides.map((slide, i) => (
            <div 
              key={slide._id || i} 
              className={`bg-[#eaeaea] rounded-[10px] p-4 flex gap-10 ${editSlideIndex === i ? 'border-2 border-blue-500' : ''}`}
            >
