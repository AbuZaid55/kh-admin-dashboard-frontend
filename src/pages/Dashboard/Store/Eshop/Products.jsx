...
const calculateTotalPrice = (product) => {
  ...
  diamondGrade.map((grade, diamondIndex) => {
    goldList.map((gold) => {
      ...
      prices.push({carat: gold.carat, grade: grade, totalDiamondWeight:totalDiamondWeight[diamondIndex],total_gold_weight:product.total_gold_weight,makingCharge,wastageCharge ,laborCost,pearlCost,extraFee,extraCost,diamond_discount,gold_discount,discount_on_total,gst,goldprice,diamondprice,goldPriceAfterDiscount,diamondPriceAfterDiscount,subTotal,totalAfterDiscount,gstAmount,finalTotalPrice});
    })
  })
  return prices;
};
        ...
        <table className="min-w-full bg-white shadow-md rounded-lg">
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {productList.map((product) => {
              ...
              return <tr key={product._id} className="border-b border-gray-200 hover:bg-gray-50">
                <td className="py-3 px-6 flex items-center">
                  <img src={product.images1[0]?.url} alt={product.name} width={50} height={50} className="rounded" />
                  <div className="ml-3">
                    <p className="font-semibold">{product.name}</p>
                    <p className="text-gray-500 text-xs">{product.total_gold_weight}gm gold {data && data[0]?.totalDiamondWeight} carat</p>
                    <p className="text-gray-500 text-xs">{data && data[0]?.carat} gold {data && data[0]?.grade} grade</p>
                  </div>
                </td>
                ...
                <td className="py-3 px-6 text-white">{
                  product.stock == 0 ? <span className="bg-red-700 w-7 flex items-center justify-center rounded-full aspect-square">{product.stock}</span> : (product.stock > 0 && product.stock < 10) ? <span className="bg-yellow-500 w-7 flex items-center justify-center rounded-full aspect-square">{product.stock}</span> : <p className="bg-green-700 w-7 flex items-center justify-center rounded-full aspect-square">{product.stock}</p>
                }</td>
                <td className="py-3 px-6">
                  <button className="text-2xl mr-3"><Link to={`/dashboard/store/eshop/update-product?productId=${product._id}`}><BiEditAlt className=' text-[#676767] cursor-pointer' /></Link></button>
                  <button onClick={()=>{deleteProduct(product._id)}} className="text-2xl"><RiDeleteBin5Line className=' text-red-500 cursor-pointer' /></button>
                </td>
              </tr>
            })}
          </tbody>
